...
class DaraForm {
  ...
  addRow(field) {
    ...
    let rednerTemplate = '';
    if (field.render) {
      rednerTemplate = field.render.call(null, field);
    } else {
      rednerTemplate = this.rowTemplate(field);
    }
    rowElement.innerHTML = rednerTemplate;
    this.formElement.appendChild(rowElement); // Append the toast element
  }

  rowTemplate(field) {
    let fieldHtml = '';
    if (field.type == 'number') {
      fieldHtml = `<input type="number" name="${field.name}" class="form-field number" />`;
    } else if (field.renderType == 'textarea') {
      fieldHtml = `<textarea name="${field.name}" class="form-field textarea"></textarea>`;
    } else if (field.renderType == 'dropdown') {
      fieldHtml = `<select name="${field.name}" class="form-field dropdown">`;
      field.value.forEach(val => {
        fieldHtml += `<option value="${val.code}">${val.value}</option>`;
      });
      fieldHtml += `</select>`;
    } else {
      fieldHtml = `<input type="text" name="${field.name}" class="form-field text" />`;
    }
    return `
